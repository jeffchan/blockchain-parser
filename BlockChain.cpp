...
#define MAX_BLOCK_TRANSACTION 1024		// never expect more than 1024 transactions per block.
...
public:
	...
	bool readTransation(BlockChain::BlockTransaction &transaction)
	{
		bool ret = false;
		...
		assert( transaction.transactionVersionNumber == 1 || transaction.transactionVersionNumber == 2 );
	...
	bool processBlockData(const void *blockData,uint32_t blockLength)
	{
		...
		if ( transactionCount < MAX_BLOCK_TRANSACTION )
		{
			...
			for (uint32_t i=0; i<transactionCount; i++)
			{
				...
				if ( !readTransation(b) )	// Read the transaction; if it failed; then abort processing the block chain
				{
					ret = false;
					break;
				}
	...
	const BlockChain::BlockTransaction *processTransactionData(const void *transactionData,uint32_t transactionLength)
	{
		...
		if ( !readTransation(*ret) )	// Read the transaction; if it failed; then abort processing the block chain
		{
			ret = NULL;
		}
		return ret;
...
public:
	...
	bool openBlock(void)
	{
		...
		char scratch[512];
		sprintf(scratch,"%sblk%05d.dat", mRootDir, mBlockIndex );	// get the filename
	...
	virtual const Block *readBlock(void)
	{
		...
		// If this is the very first time; read in the first four blocks; if this fails, then we don't have a
		// valid block chain data file. We expect at least 4 blocks.
		// It reads multiple blocks in so that it can resolve the 'nextHash' and 'currentHash' and 'previousHash' values for each block.
		switch ( mReadCount )
		{
			case 0:
				if ( readBlock(mBlocks[0]) &&
					 readBlock(mBlocks[1]) &&
					 readBlock(mBlocks[2]) &&
					 readBlock(mBlocks[3]) )
				{
					mBlockBase = 0;
					mPreviousBlock	= &mBlocks[0];
					mCurrentBlock	= &mBlocks[1];
					mNextBlock1		= &mBlocks[2];
					mNextBlock2		= &mBlocks[3];

					mNextBlock1->blockHash = mNextBlock2->previousBlockHash;
					mNextBlock1->nextBlockHash = NULL; // unknown at this time.

					mCurrentBlock->blockHash = mNextBlock1->previousBlockHash;
					mCurrentBlock->nextBlockHash = mNextBlock1->blockHash;

					mPreviousBlock->blockHash = mCurrentBlock->previousBlockHash;
					mPreviousBlock->nextBlockHash = mCurrentBlock->blockHash;
					mPreviousBlock->previousBlockHash = NULL; // there is no previous block hash

					ret = mPreviousBlock; // since this is the first time in; we return the 'prevoius' block pointer.
				}
				break;
			case 1:
				ret = mCurrentBlock; // return the current block
				break;
			default:
				{
					mPreviousBlock = mCurrentBlock;
					mCurrentBlock = mNextBlock1;
					mNextBlock1 = mNextBlock2;
					if ( mNextBlock2 )
					{
						mNextBlock2 = &mBlocks[mBlockBase];
						mBlockBase++;
						if ( mBlockBase == 4 )
						{
							mBlockBase = 0;
						}
						if ( readBlock(*mNextBlock2) )
						{
							mNextBlock1->blockHash = mNextBlock2->previousBlockHash;
							mNextBlock1->nextBlockHash = NULL; // unknown at this time.
	...
	virtual bool readBlock(BlockImpl &block)
	{
		...
		if ( fph )
		{
			...
			if ( r == 1 )	// Ok, this is a valid block, let's continue
			{
				...
				block.fileOffset = (uint32_t)ftell(fph);
				r = fread(&block.blockLength,sizeof(block.blockLength),1,fph); // read the length of the block
				if ( r == 1 )
				{
					assert( block.blockLength < MAX_BLOCK_SIZE ); // make sure the block length does not exceed our maximum expected ever possible block size
					if ( block.blockLength < MAX_BLOCK_SIZE )
					{
						uint8_t *blockData = getNextBlockData();
						r = fread(blockData,block.blockLength,1,fph); // read the rest of the block (less the 8 byte header we have already consumed)
						if ( r == 1 )
						{
	...
	virtual void printBlock(const Block *block) // prints the contents of the block to the console for debugging purposes
	{
		...
		for (uint32_t i=0; i<block->transactionCount; i++)
		{
			...
			for (uint32_t i=0; i<t.outputCount; i++)
			{
				...
				{
					char scratch[256];
					bool ok = BLOCKCHAIN_BITCOIN_ADDRESS::bitcoinPublicKeyToAscii(output.publicKey,scratch,256);
					if ( ok )
					{
						printf("PublicKey: %s\r\n", scratch );
					}
	...
	virtual const Block * getPreviousBlock(void)
	{
		return mPreviousBlock;
	}

	virtual const Block * getCurrentBlock(void)
	{
		return mCurrentBlock;
	}

	virtual const Block * getNextBlock(void)
	{
		return mNextBlock1;
	}

	virtual const Block * getNextBlockAfter(void)
	{
		return mNextBlock2;
	}
	...
	virtual const Block * processSingleBlock(const void *blockData,uint32_t blockLength)
	{
		...
		if ( blockLength < MAX_BLOCK_SIZE )
		{
			...
			mSingleBlock.processBlockData(blockData,blockLength);
	...
	virtual const BlockTransaction *readSingleTransaction(uint32_t fileIndex,uint32_t fileOffset,uint32_t transactionLength)
	{
		const BlockTransaction *ret = NULL;
	...
	BlockImpl			*mPreviousBlock;
	BlockImpl			*mCurrentBlock;
	BlockImpl			*mNextBlock1;
	BlockImpl			*mNextBlock2;
